				...
				$("#btn-1").on("click",function(){
					...
					$.ajax({
					     data: {user: user, 
					     	    pass: passwd,
					     	    agencia: agencia},
	   					 url:  'http://apirest/usuarios/login/',
	    				 type: 'POST',
			...
			function mostrarResultado(datos){
				if(!entrado){
					...
					if(datos["result"] == "true"){
						$("#container ul").append('<li>El usuario se ha logeado correctamente</li>');
					}else{
						$("#container ul").append('<li>El usuario no se ha podido logear</li>');
					}
