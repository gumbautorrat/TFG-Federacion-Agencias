...
function cargaPagina(pagina,tipo,agencia)
{
	...
	$.ajax({
		...
		type:"POST",
		dataType:"json",
		//url:  'http://localhost/apirest_federacion/inmuebles_paginado/' + id_agencia,
	...
	controlCargaPagina(tipo,agencia);
		...
		if(data.length == 0){
			...
			$.each(data, function(ind, elem){
				...
				inmueble += "<a href=\"vistas/detalle.php?tipo="+elem.id_tipo+"&id_inmueble="+elem.id_inmueble+" \" class=\"btn btn-primary\">Leer Mas</a>";
				inmueble += "</div>";
...
// Funcion que truca la longitud de la descripción del inmueble
