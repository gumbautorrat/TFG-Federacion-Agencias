<?php 
  ...
  //Recogemos los datos del propietario seleccionado en la pag anterior
  if(isset($_GET["id"]) && isset($_GET["id_agencia"]) &&
  	 isset($_GET["nombre"]) && isset($_GET["apell1"]) && 
  	 isset($_GET["apell2"]) && isset($_GET["direccion"]) &&
  	 isset($_GET["localidad"]) && isset($_GET["provincia"]) &&
  	 isset($_GET["telefono"]) && isset($_GET["email"]))
  {
...
<head>
	...
	<title>Actualizar Propietario <?php echo TITULO ?></title>
...
<body>
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navegacion-fm">
					<ul class="nav navbar-nav">
						<li class="active"><a>Actualizar Propietario</a></li>
					</ul>
				...
				<div class="row">
					...
					<div class="col-xs-7">
						<div class="panel panel-default ">
							<div class="panel-heading"><center><h4>Actualizar Propietario</h4></center></div>
						</div>
		  			</div>
		  		</div>
		  		<br>
		  		...
		  		<form method="post" id="form" class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'] ?>">
				    ...
				    <div class="form-group">
				        <div class="col-xs-offset-4 col-xs-9">
				            <input type="submit" name="actualizar" class="btn <?php echo BTN_ACTUALIZAR_CLASS ?>" value="Enviar">
	...
	<!-- Dialog -->
	<div id="dialog" title="Actualizar Propietario" style="display:none;">
    	<p id="mensaje"></p>
    </div>
    <!-- Dialog -->
...
//Si se ha enviado el formulario para actualizar el propietario
if(isset($_POST['actualizar'])){
	...
	$dni        = $_POST["inputDni"];
  	...
  	$cliente = new ClienteWSPropietarios();
    ...
    $resultadoJSON = $cliente->sendActualizarPropietario($user,$pass,$agencia,$id,$id_agencia,$dni,$nombre,
	...
	$url_destino = "propietarios.php";

	if(strcmp($resultado->message,"Operation Succesful") == 0){
		echo "<script>mostrarMensaje(\"El propietario ha sido actualizado correctamente\",'".$url_destino."');</script>";
	}elseif(strcmp($resultado->message,"Autentication Failure") == 0){
		echo "<script>mostrarMensaje(\"No tienes permisos para actualizar propietarios o el propietario que intentas actualizar no es de tu agencia\",'".$url_destino."');</script>";
	}else{
